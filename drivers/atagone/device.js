	...
	async updateStatus() {
		...
		try {
			if (success) {
				...
				if (this.getCapabilityValue('measure_temperature') !== this.report.room_temp) { this.driver._triggers.trgTempChanged.trigger(this, { insidetemp: this.report.room_temp, outsidetemp: this.report.outside_temp }).catch(this.error).then(this.log); }

				if (this.getCapabilityValue('my_outside_temperature') !== this.report.outside_temp) { this.driver._triggers.trgOutsideTempChanged.trigger(this, { insidetemp: this.report.room_temp, outsidetemp: this.report.outside_temp }).catch(this.error).then(this.log); }

				if (this.getCapabilityValue('my_water_temp') !== this.report.dhw_water_temp) { this.driver._triggers.trgWaterTempChanged.trigger(this, { temp: this.report.dhw_water_temp }).catch(this.error).then(this.log); }

				// current setups do not have this, so check if exist.
				if (this.hasCapability('central_heating')) {
					if (this.getCapabilityValue('central_heating') !== ((this.report.boiler_status & 8) == 8)) { this.driver._triggers.trgHeatingChanged.trigger(this, { heating_status: ((this.report.boiler_status & 8) == 8) }).catch(this.error).then(this.log); }
				}

				if (this.getCapabilityValue('my_water_pressure') !== this.report.ch_water_pres) { this.driver._triggers.trgPressureChanged.trigger(this, { pressure: this.report.ch_water_pres }).catch(this.error).then(this.log); }

				if (this.getCapabilityValue('my_burning_hours') !== formatValue(this.report.burning_hours)) { this.driver._triggers.trgBurningHoursChanged.trigger(this, { burninghours: formatValue(this.report.burning_hours) }).catch(this.error).then(this.log); }

				if (this.getCapabilityValue('my_heatingwater_temp') !== this.report.ch_water_temp) { this.driver._triggers.trgHeatingWaterTempChanged.trigger(this, { temp: this.report.ch_water_temp }).catch(this.error).then(this.log); }

				if (this.getCapabilityValue('my_heatingreturnwater_temp') !== this.report.ch_return_temp) { this.driver._triggers.trgHeatingReturnWaterTempChanged.trigger(this, { temp: this.report.ch_return_temp }).catch(this.error).then(this.log); }
			}			
		} catch (error) {
			this.log(error);
		}
